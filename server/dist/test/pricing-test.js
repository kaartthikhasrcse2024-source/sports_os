...
function assertObj(name, { original, final, breakdown }, expFinal, expBreakdownLength) {
...
async function runTests() {
    ...
    assertObj('Normal Weekday Off-Peak', (0, pricing_1.calculateYieldPrice)(d1.toISOString(), base, 1.2, 1.15, true, null), 100, 0);
    // 2. Weekend Off-Peak (Saturday 2PM) 
    const d2 = new Date('2026-08-15T14:00:00');
    assertObj('Weekend Off-Peak', (0, pricing_1.calculateYieldPrice)(d2.toISOString(), base, 1.2, 1.15, true, null), 115, 1);
    // 3. Weekday Peak Hour (Wednesday 7PM / 19:00)
    const d3 = new Date('2026-08-12T19:00:00');
    assertObj('Weekday Peak Hour', (0, pricing_1.calculateYieldPrice)(d3.toISOString(), base, 1.2, 1.15, true, null), 120, 1);
    // 4. Weekend Peak Hour (Saturday 7PM)
    const d4 = new Date('2026-08-15T19:00:00');
    assertObj('Weekend Peak Hour', (0, pricing_1.calculateYieldPrice)(d4.toISOString(), base, 1.2, 1.15, true, null), 138, 2);
    // 5. Weather Discount (Weekday Off-peak, Rain, outdoor)
    const weatherRain = { temperature: 20, rain: 5 };
    assertObj('Weather Discount (Rain Outdoor)', (0, pricing_1.calculateYieldPrice)(d1.toISOString(), base, 1.2, 1.15, true, weatherRain), 80, 1);
    // 6. Weather Extreme Heat (Weekday Off-peak, Heat, outdoor)
    const weatherHeat = { temperature: 38, rain: 0 };
    assertObj('Weather Discount (Heat Outdoor)', (0, pricing_1.calculateYieldPrice)(d1.toISOString(), base, 1.2, 1.15, true, weatherHeat), 80, 1);
    // 7. Weather Discard (Weekday Off-peak, Rain, but INDOOR)
    assertObj('Weather Rejected (Indoor facility protection)', (0, pricing_1.calculateYieldPrice)(d1.toISOString(), base, 1.2, 1.15, false, weatherRain), 100, 0);
