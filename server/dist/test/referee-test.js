...
async function runTest() {
    ...
    await db_1.default.query(`INSERT INTO auth.users (id, instance_id, aud, role, email) VALUES ($1, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'p1@test.com') ON CONFLICT DO NOTHING`, [p1Id]);
    await db_1.default.query(`INSERT INTO auth.users (id, instance_id, aud, role, email) VALUES ($1, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'p2@test.com') ON CONFLICT DO NOTHING`, [p2Id]);
    await db_1.default.query(`INSERT INTO auth.users (id, instance_id, aud, role, email) VALUES ($1, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'ref@test.com') ON CONFLICT DO NOTHING`, [refId]);
    // Insert corresponding profiles
    await db_1.default.query(`INSERT INTO profiles (id, name, availability_status, sport_type, position, win_rate) VALUES ($1, 'Player One', 'looking_for_team', 'basketball', 'guard', 0) ON CONFLICT (id) DO NOTHING`, [p1Id]);
    await db_1.default.query(`INSERT INTO profiles (id, name, availability_status, sport_type, position, win_rate) VALUES ($1, 'Player Two', 'not_available', 'basketball', 'center', 0) ON CONFLICT (id) DO NOTHING`, [p2Id]);
    await db_1.default.query(`INSERT INTO profiles (id, name, role) VALUES ($1, 'Ref Bob', 'referee') ON CONFLICT (id) DO NOTHING`, [refId]);
    // 2. Setup mock tournament boundary
    const tRes = await db_1.default.query(`INSERT INTO tournaments (name, format, max_teams, start_date) VALUES ('Demo Bracket', 'single_elim', 4, now()) RETURNING id`);
    const tId = tRes.rows[0].id;
    const fRes = await db_1.default.query(`INSERT INTO tournament_teams (tournament_id, team_name) VALUES ($1, 'Team A') RETURNING id`, [tId]);
    const sRes = await db_1.default.query(`INSERT INTO tournament_teams (tournament_id, team_name) VALUES ($1, 'Team B') RETURNING id`, [tId]);
    const tA = fRes.rows[0].id;
    const tB = sRes.rows[0].id;
    const mRes = await db_1.default.query(`INSERT INTO bracket_matches (tournament_id, round, match_index, team_a_id, team_b_id, referee_id) VALUES ($1, 1, 0, $2, $3, $4) RETURNING id`, [tId, tA, tB, refId]);
