...
router.post('/registration', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (!number_of_turfs || isNaN(parseInt(number_of_turfs)) || parseInt(number_of_turfs) < 1)
            errors.number_of_turfs = 'Must be > 0';
...
router.use((0, auth_1.requireRole)(['TURF_OWNER']));
router.get('/slots', async (req, res) => {
    try {
        const userId = req.user.sub || req.user.id;
        // 1. Get owner's courts
        const courtsRes = await db_1.default.query(`
        ...
        const slotsExist = await db_1.default.query(`
...
router.post('/slots/override', async (req, res) => {
    try {
        ...
        await db_1.default.query(`UPDATE slots SET status = $1 WHERE id = $2`, [newStatus, id]);
...
router.get('/roster', async (req, res) => {
    try {
        ...
        const facRes = await db_1.default.query('SELECT id FROM facilities WHERE owner_id = $1', [userId]);
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        const occupancyResult = await db_1.default.query(`
            SELECT 
