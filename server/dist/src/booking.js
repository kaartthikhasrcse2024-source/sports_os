...
router.post('/group-reserve', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const insertPromises = contributor_ids.map((cid) => {
            return client.query(`INSERT INTO booking_contributions (booking_id, user_id, amount_owed, status) VALUES ($1, $2, $3, 'pending')`, [booking_id, cid, splitAmount]);
        });
...
router.post('/contributions/:id/checkout', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    try {
        const contributionRes = await db_1.default.query('SELECT * FROM booking_contributions WHERE id = $1', [id]);
...
router.post('/contributions/:id/verify', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    // In production, verify razorpay_signature here
    try {
        await db_1.default.query("UPDATE booking_contributions SET status = 'paid' WHERE id = $1", [id]);
    ...
    try {
        ...
        const formatted = await Promise.all(result.rows.map(async (row) => {
            ...
            if (condition === undefined && row.lat && row.lng) {
                condition = await (0, pricing_1.fetchFacilityWeather)(row.lat, row.lng) || null;
            ...
            const yieldDetail = (0, pricing_1.calculateYieldPrice)(row.start_time, Number(row.base_price_per_hour) || 1500, Number(row.peak_hour_multiplier) || 1.2, Number(row.weekend_multiplier) || 1.15, row.is_outdoor, condition);
            return {
                ...row,
