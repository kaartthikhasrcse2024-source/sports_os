...
router.post('/create-order', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        if (metadata?.payment_type === 'SPLIT_ESCROW' && metadata.contribution_id !== 'dummy_c') {
            const contribRes = await db_1.default.query('SELECT status FROM booking_contributions WHERE id = $1', [metadata.contribution_id]);
        ...
        const order = await payment_1.razorpay.orders.create({
...
router.post('/verify-signature', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const fetchedOrder = await payment_1.razorpay.orders.fetch(razorpay_order_id);
        if (!fetchedOrder)
        ...
        if (payment_type === 'CASUAL_BOOKING') {
            ...
            await client.query(`INSERT INTO transactions (type, actor_id, actor_role, gross_amount, platform_fee, net_amount, related_booking_id, status)
                 VALUES ('BOOKING', (SELECT owner_id FROM facilities f JOIN courts c ON c.facility_id = f.id JOIN slots s ON s.court_id = c.id WHERE s.id = $1 LIMIT 1), 'TURF_OWNER', $2, $3, $4, $5, 'COMPLETED')`, [bookRes.rows[0].slot_id, totalGross, platformFee, netAmount, booking_id]);
            ...
            if (myContrib.rows[0].user_id !== auth_user)
            ...
            if (allPaid) {
                ...
                if (bRes2.rows.length) {
                    ...
                    await client.query(`INSERT INTO transactions (type, actor_id, actor_role, gross_amount, platform_fee, net_amount, related_booking_id, status)
                        VALUES ('BOOKING', (SELECT owner_id FROM facilities f JOIN courts c ON c.facility_id = f.id JOIN slots s ON s.court_id = c.id WHERE s.id = $1 LIMIT 1), 'TURF_OWNER', $2, $3, $4, $5, 'COMPLETED')`, [bRes2.rows[0].slot_id, totalGross, platform_fee, net_amount, booking_id]);
                    await (0, notifications_1.createNotification)(client, {
                        recipientId: auth_user,
                        type: 'BOOKING_CONFIRMED',
            ...
            await client.query(`INSERT INTO transactions (type, actor_id, actor_role, gross_amount, platform_fee, net_amount, status)
                 VALUES ('BOOKING', (SELECT owner_id FROM facilities f JOIN venue_lease_requests vlr ON vlr.facility_id = f.id WHERE vlr.id = $1), 'TURF_OWNER', $2, $3, $4, 'COMPLETED')`, [lease_id, gross_amount, platform_fee, net_amount]);
        }
            ...
            if (!tournament_id || !entry_id)
                throw new Error('Tournament ID and Entry ID are required.');
            ...
            await client.query(`INSERT INTO transactions (type, actor_id, actor_role, gross_amount, platform_fee, net_amount, related_tournament_id, status)
                 VALUES ('TOURNAMENT_ENTRY', (SELECT owner_id FROM facilities f JOIN tournaments t ON t.facility_id = f.id WHERE t.id = $1 LIMIT 1), 'TOURNAMENT_ORGANIZER', $2, $3, $4, $1, 'COMPLETED')`, [tournament_id, gross_amount, platform_fee, net_amount]);
        }
        await client.query('COMMIT');
