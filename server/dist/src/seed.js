...
async function insertUser(id, email, name, role, phone, is_verified = true) {
    // 1. auth.users
    await pool.query(`INSERT INTO auth.users (id, email) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING`, [id, email]);
    // 2. profiles
    await pool.query(`INSERT INTO profiles (id, name, role, verification_status, created_at) 
         VALUES ($1, $2, $3, $4, now()) 
         ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, role = EXCLUDED.role, verification_status = EXCLUDED.verification_status`, [id, name, role, is_verified ? 'VERIFIED' : 'PENDING']);
}
async function insertFacility(ownerId, name, address, lat, lng, amenities) {
    const res = await pool.query(`INSERT INTO facilities (owner_id, name, address, location, amenities) 
         VALUES ($1, $2, $3, ST_SetSRID(ST_Point($5, $4), 4326), $6) 
         RETURNING id`, [ownerId, name, address, lat, lng, amenities]);
    return res.rows[0].id;
}
async function insertCourt(facilityId, name, sportType, rate) {
    const res = await pool.query(`INSERT INTO courts (facility_id, name, sport_type, base_price_per_hour) VALUES ($1, $2, $3, $4) RETURNING id`, [facilityId, name, sportType, rate]);
    return res.rows[0].id;
}
async function main() {
    ...
    try {
        ...
        // Create Tournaments
        await pool.query(`INSERT INTO tournaments (facility_id, name, format, max_teams, start_date) 
             VALUES ($1, 'Chennai Premier Futsal Cup 2026', 'single_elim', 8, now() + interval '7 days')`, [fac1]);
        await pool.query(`INSERT INTO tournaments (facility_id, name, format, max_teams, start_date) 
             VALUES ($1, 'Coastal Badminton Championship', 'round_robin', 16, now() + interval '14 days')`, [fac2]);
        // --- 3. MOCK PLAYERS ---
