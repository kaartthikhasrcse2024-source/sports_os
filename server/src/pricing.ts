export interface OMWestCondition {
    temperature: number;
    rain: number;
}
...
export function calculateYieldPrice(
    startTimeStr: string,
    basePrice: number,
    peakMultiplier: number = 1.2,
    weekendMultiplier: number = 1.15,
    isOutdoor: boolean = true,
    weather: OMWestCondition | null = null
): { original: number, final: number, breakdown: string[] } {
...
export async function fetchFacilityWeather(lat: number, lng: number): Promise<OMWestCondition | null> {
    try {
