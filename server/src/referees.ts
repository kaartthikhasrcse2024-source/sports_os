...
router.post('/matches/:id/live-match-scorecard', async (req, res) => {
    ...
    try {
        ...
        const mNode = await pool.query(`SELECT team_a_id, team_b_id, referee_id, next_match_id, match_index FROM bracket_matches WHERE id = $1`, [id]);
        const match = mNode.rows[0];
        if (!match) return res.status(404).json({ error: 'Bracket Match mapped entity not found' });
        ...
        try {
            ...
            for (const s of stats) {
                await client.query(
                    `INSERT INTO match_stats (match_id, player_id, goals, points, fouls, yellow_cards, red_cards, minutes_played)
                      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
                    [id, s.player_id, s.goals || 0, s.points || 0, s.fouls || 0, s.yellow_cards || 0, s.red_cards || 0, s.minutes || 0]
                );
                // Assuming unified measurement parameter, add to tracking metrics
                if (s.team_id === match.team_a_id) teamAScore += (s.points || s.goals || 0);
                if (s.team_id === match.team_b_id) teamBScore += (s.points || s.goals || 0);
            }

            // Derive explicit winner tracking
            let winner_id = null;
            if (teamAScore > teamBScore) winner_id = match.team_a_id;
            else if (teamBScore > teamAScore) winner_id = match.team_b_id;
