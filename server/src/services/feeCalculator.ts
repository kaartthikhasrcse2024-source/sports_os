 ...
 */
export function calculateConvenienceFee(bookingAmountBase: number): FeeResult {
    if (!Number.isInteger(bookingAmountBase)) throw new Error('Input must be absolute integer cents');
 ...
 */
export function calculateVenueCommission(totalCollectedAmount: number): FeeResult {
    if (!Number.isInteger(totalCollectedAmount)) throw new Error('Input must be absolute integer cents');
 ...
 */
export function calculateTournamentCommission(entryFeeAmount: number): FeeResult {
    if (!Number.isInteger(entryFeeAmount)) throw new Error('Input must be absolute integer cents');
