...
import { Building, ShieldCheck, ArrowRight, LayoutDashboard, ChevronLeft } from 'lucide-react';
import { getSportImage } from '../utils/sportsImages';
import { usePageEnter } from '../utils/animations';

export default function OwnerAuth({ mode = 'login' }: { mode?: 'login' | 'signup' }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isSignUp) {
            const { error } = await supabase.auth.signUp({
                ...
                password,
                options: {
                    data: { full_name: fullName, role: 'TURF_OWNER' }
                }
            });
    ...
    return (
        <div className={`min-h-screen flex bg-slate-50 font-sans text-slate-900 selection:bg-emerald-500 selection:text-white ${pageClass}`}>
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-white items-end p-12">
                <img
                    src={getSportImage('stadium', 1)}
                    alt="Turf Facility"
                    className="absolute inset-0 w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/40 to-transparent mix-blend-multiply opacity-50" />
                ...
                <div className="relative z-10 bg-white/95 backdrop-blur-md p-8 rounded-3xl shadow-2xl max-w-lg border border-white/20">
                    ...
                    <p className="text-slate-600 font-medium">
                        Join the premier network of sports facilities. Streamline bookings, maximize occupancy, and unlock powerful revenue analytics.
                    </p>
                </div>
            </div>
                ...
                <div className="w-full max-w-md relative z-10 pt-[50vw] sm:pt-10 lg:pt-0">
                    ...
                    <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-xl border border-slate-100">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
                                    {isSignUp ? 'Establish Facility.' : 'Facility Access.'}
                                </h2>
                                <p className="text-slate-500 font-medium mt-1">
                                    {isSignUp ? 'Partner with Sports OS.' : 'Welcome back, Owner.'}
                                </p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            {isSignUp && (
                                <div>
                                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Legal Owner Name</label>
                                    <input
                                        type="text"
                                        value={fullName}
                            ...
                            >
                                <span className="uppercase tracking-widest text-xs flex-1 text-center">
                                    {loading ? 'Processing...' : (!isSignUp ? 'Authorize Access' : 'Register Turf & Owner')}
                                </span>
                        ...
                        <div className="mt-8 text-center bg-slate-50 p-4 rounded-xl">
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">
                                {!isSignUp ? (
                                    <>Not a facility partner? <button type="button" onClick={() => { setIsSignUp(true); setError(''); setSuccessMessage(''); }} className="text-emerald-600 hover:text-emerald-700 transition-colors ml-1 underline decoration-emerald-600/30 underline-offset-2">Register Now</button></>
                                ) : (
                                    <>Existing facility? <button type="button" onClick={() => { setIsSignUp(false); setError(''); setSuccessMessage(''); }} className="text-emerald-600 hover:text-emerald-700 transition-colors ml-1 underline decoration-emerald-600/30 underline-offset-2">Sign In</button></>
                                )}
                            </p>
                        </div>
                    </div>
