...
export default function VerifyPhone() {
    ...
    return (
        <div className="bg-dark-800 p-6 rounded-2xl border border-dark-700 shadow-xl max-w-sm w-full mx-auto text-center">
            <div className="flex justify-center mb-4 text-primary-500">
                <Smartphone size={32} />
            </div>
            <h2 className="text-xl font-black text-white mb-2">Two-Factor Shield</h2>
            <p className="text-gray-400 text-sm mb-6 font-medium">Link your cellular array to authorize sensitive platform bookings.</p>
            ...
            {step === 1 ? (
                <form onSubmit={handleSendOTP} className="space-y-4 text-left">
                    <div>
                        <label className="block text-gray-400 text-xs font-black uppercase mb-2">Mobile Interface</label>
                        <input
                            type="tel"
                            value={phone}
                ...
                <form onSubmit={handleVerify} className="space-y-4 text-left">
                    <div>
                        <label className="block text-gray-400 text-xs font-black uppercase mb-2">Decryption Hash (OTP)</label>
                        <input
                            type="text"
                            value={otpCode}
                            onChange={(e) => setOtpCode(e.target.value)}
                            className="w-full bg-dark-900 border border-dark-700 rounded-xl p-3 text-white text-center tracking-widest font-mono text-2xl focus:border-primary-500 outline-none"
                            placeholder="000-000"
                            required
                        />
                    </div>
